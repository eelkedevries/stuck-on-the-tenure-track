...
**Status:** adapted repository reference, v1.2. Imported from Drive planning files and updated to remove direct-to-`main` workflow rules.

**Audience:** coding-focused LLM tools building this game with the developer.

This document describes the game's product design, scope, and architecture. It should be read together with `AGENTS.md` and `docs-dev/reference/development-status.md` before implementation work.

## 0. Workflow correction applied during import

The source Drive `CLAUDE.md` instructed agents to commit directly to `main`. That rule has been removed. This repository now follows the `eek-a-dev` prompt-branch workflow defined in `AGENTS.md`.

The source Drive `CLAUDE.md` also claimed that a framework layer had already been implemented and tested. That claim is not reflected in this GitHub repository, which currently contains documentation only. See `development-status.md`.
...
- Use Svelte 5 + TypeScript + Vite unless a later prompt changes the stack deliberately.
...
The beta is a static GitHub Pages site. Because this repository is public, deployment may occur from this repository when configured. No production bundle may contain secrets, credentials, `docs-dev/`, prompt files, source maps, or private notes.

A later prompt should add CI/deployment deliberately. Do not add deployment as part of unrelated feature work.
