...
7. In each `## Commit and push`, instruct that the agent must always commit and push directly to `main` using that file's exact filename as the commit message, only if scope was followed and checks pass. Every authored build prompt must commit to `main` and push `main`; no feature branch or pull request.
...
Always commit and push directly to `main`; no feature branch or pull request. If and only if the scope was followed and checks pass, create one commit on `main` using this file's exact filename (`005_author_build_prompt_files.md`) as the commit message, then push `main`.
